...
export class SymbolPrecisionManager {
  ...
  public async loadExchangeInfo(exchangeInfoData: any): Promise<void> {
    try {
      ...
      if (
        !exchangeInfoData.symbols ||
        !Array.isArray(exchangeInfoData.symbols)
      ) {
      ...
      for (const symbolData of exchangeInfoData.symbols) {
        try {
          ...
          logger.warn(
            `Failed to parse symbol data for ${symbolData.symbol}:`,
            error
          );
        }
      }
        ...
        switch (filter.filterType) {
          ...
          case "MIN_NOTIONAL":
            minNotional = parseFloat(
              filter.notional || filter.minNotional || "5.0"
            );
            break;
        }
      }
  ...
  public formatQuantity(quantity: number, symbol: string): string {
    ...
    const rounded =
      Math.round(quantity / symbolInfo.stepSize) * symbolInfo.stepSize;
  ...
  public formatPrice(price: number, symbol: string): string {
    ...
    const rounded =
      Math.round(price / symbolInfo.tickSize) * symbolInfo.tickSize;
  ...
  public validateNotional(
    ...
    if (notional < symbolInfo.minNotional) {
      return {
        valid: false,
        reason: `Notional value ${notional.toFixed(2)} below minimum ${
